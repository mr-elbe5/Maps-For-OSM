...
class FileBasedData : PlaceItem{
    ...
    var fileRecord: CKRecord{
        get{
            ...
            record[FileData.fileNameKey] = fileName
            record[FileData.dataKey] = data
            return record
        }
    }
    ...
    func loadFileFromICloud(record: CKRecord){
        if let data = record.value(forKey: FileData.dataKey) as? Data, let fileName : String = record.value(forKey: FileData.fileNameKey) as? String{
